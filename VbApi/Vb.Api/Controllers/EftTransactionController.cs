...
namespace VbApi.Controllers
{
    ...
    public class EftTransactionController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ApiResponse<EftTransactionResponse>> Post([FromBody] EftTransactionRequest address)
        {
            var operation = new CreateEftTransactionCommand(address);
            var result = await mediator.Send(operation);
            return result;
        }

        [HttpPut("{id}")]
        public async Task<ApiResponse> Put(int id, [FromBody] EftTransactionRequest address)
        {
            var operation = new UpdateEftTransactionCommand(id, address);
            var result = await mediator.Send(operation);
            return result;
        }
